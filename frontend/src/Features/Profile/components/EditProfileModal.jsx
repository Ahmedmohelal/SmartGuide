...
import { X, Save, Camera, Upload, Plus } from "lucide-react";
...
export default function EditProfileModal({ user, onClose, onSave }) {
  ...
  const [formData, setFormData] = useState({
    ...
    bio: user?.bio || "",
    cities: Array.isArray(user?.cities)
      ? user.cities
      : user?.cities
      ? [user.cities]
      : [],
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 flex-1">
          ...
          <div className="flex flex-col items-center pb-6 border-b">
            <div className="relative">
              ...
              <label
                ...
                className="absolute bottom-0 right-0 bg-egypt-teal text-white p-2 rounded-full cursor-pointer hover:bg-egypt-teal/90 transition-colors shadow-lg"
              >
                <Upload size={16} />
              </label>
          ...
          <div className="flex gap-3 pt-4">
            <button
              ...
              className="flex-1 bg-egypt-teal text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2"
            >
              {isUploading ? "Saving..." : (
                <>
