...
export default function GuideVerification() {
  ...
  // استلام البيانات الكاملة من صفحة Register
  ...
  // --- دالة إرسال الطلب الكامل للباك إند ---
  const handleFormSubmit = async () => {
    ...
    if (!basicInfo) {
      setError("Missing registration info. Please go back.");
      return;
    }
    ...
    try {
      ...
      // 1. إضافة البيانات النصية (مطابقة للسواجر)
      ...
      // 2. إضافة الملفات (مطابقة للسواجر)
      formData.append("NationalIdImage", idFile);
      formData.append("GuideLicenselImage", licenseFile); 

      // 3. استدعاء API التسجيل
      await authService.register(formData);

      // --- التعديل هنا: التوجيه للـ Login مباشرة لأن الـ OTP لسه مخلصش عند الباك إند ---
      alert("Registration submitted successfully! You can now login to your account.");
      navigate("/login"); 
    ...
    return (
      <div className="mb-8">
        ...
        >
          ...
          <input id={id} type="file" hidden onChange={(e) => handleSelect(e, setFile)} />
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="relative bg-white rounded-2xl shadow-lg w-full max-w-3xl p-6 md:p-10">
        ...
        <button
          onClick={() => navigate("/register", { state: { selectedRole: "guide" } })}
          className="absolute top-4 right-4 p-2 rounded-full hover:bg-gray-100 transition"
        >
          <X size={20} className="text-gray-600" />
        </button>

        <h2 className="text-2xl font-bold mb-1 text-gray-800 text-center">Verify Your Profile</h2>
        <p className="text-gray-500 text-sm mb-8 text-center uppercase tracking-widest font-bold">Step 2: Document Upload</p>
        ...
        <UploadBox id="idInput" file={idFile} setFile={setIdFile} Icon={IdCard} label="National ID Image" />
        <UploadBox id="licenseInput" file={licenseFile} setFile={setLicenseFile} Icon={File} label="Tour Guide License Image" />
